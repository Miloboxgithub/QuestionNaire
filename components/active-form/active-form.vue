			...
			<block v-for="(item, index) in formData" :key="item.id">
				<view class="form-item flex-row--c">
                     ...
                     ">
						...
						<view class="line-right flex-row-sb-c pr20" v-if="item.type === 'code'">
							...
							<view style="width:250rpx">
								<u-button size="mini" type="primary" :disabled="item.disabled||isSend"
									@click="sendCode(item)">
									{{ codeFont }}
								</u-button>
							</view>
					...
					<template v-else-if="item.type === 'select'">
						...
						<u-action-sheet :actions="item.list" :title="item.title" safeAreaInsetBottom round="40"
							cancelText="取消" :show="item.show" @select="selectConfirm($event,item)"
					...
					<block v-else-if="item.type === 'datetime'">
						...
						<u-datetime-picker :show="item.show" :actions="item.list" :title="item.title"
							:mode="item.mode||'date'"
							...
							@close="selectClose(item)" @cancel="selectClose(item)"></u-datetime-picker>
					</block>
					...
					<template v-else-if="item.type === 'calendar'">
						...
						<u-calendar :show="item.show" :title="item.title" :round="10" :mode="item.mode||'single'"
							cancelText="取消" @confirm="selectCalendar($event,item)" @close="selectClose(item)"
							:defaultDate="item.defaultDate"></u-calendar>
					</template>
					...
					<view class="textarea-box" v-else-if="item.type === 'textarea'">
						...
						<view class="line-bottom-textarea">
							...
							<textarea style="color: #a7a7a7; font-size: 24rpx" auto-height :maxlength="-1"
					...
					<view class="line-col" v-else-if="item.type === 'radio'">
						...
						<view class="line-bottom-select" :style="{ 'padding-left': num ? '60rpx' : '34rpx' }">
							<u-radio-group v-model="form[item.rules.name]" @change="radioChange($event, index)"
								placement="row">
								<u-radio :size="35" :icon-size="35" :label-size="25" shape="circle"
					...
					<view class="line-col" v-else-if="item.type === 'checkbox'">
						...
						<view class="line-bottom-select pr20" :style="{ 'padding-left': num ? '60rpx' : '34rpx' }">
							<u-checkbox-group v-model="form[item.rules.name]"
					...
					<view class="line" v-else-if="item.type === 'mobile'">
						...
						<view class="line-right pr20">
							...
							<view style="width:200rpx" v-if="item.oneKeyPhone">
								<u-button size="mini" type="primary" open-type="getPhoneNumber"
